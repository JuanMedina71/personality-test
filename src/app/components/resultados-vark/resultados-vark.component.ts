...
@Component({
  selector: 'app-resultados-vark',
  templateUrl: './resultados-vark.component.html',
  styleUrls: ['./resultados-vark.component.css']
})
export class ResultadosVarkComponent {
  ...
  buscarDatosPorNombre() {
    ...
    this.connectionDBService.getDatosPorNombreVark(nombre).subscribe((datos) => {
  ...
  generarPDF(resultado: any) {
    ...
    addText(`Resultado: ${resultado.tipoPredominante}`, 10);
  
    // Añade la etiqueta "Descripción:" una vez
    addText(`Descripción:`, 10);
  
    // Divide la descripción en líneas más cortas si es necesario
    const descripcionLines = pdf.splitTextToSize(resultado.descripcionResultado, pdf.internal.pageSize.width - 2 * margin);
